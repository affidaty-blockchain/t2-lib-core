...
import { BulkNodeTxData } from '../../../src/transaction/bulkNodeTxData';
...
describe('bulk node tx data class', () => {
    test('constructor', async () => {
        let data: BulkNodeTxData;
        expect(() => { data = new BulkNodeTxData('wrong'); }).toThrow();
        data = new BulkNodeTxData(TxSchemas.EMPTY_TX);
        expect(data.typeTag).toEqual(SignableTypeTags.EMPTY_TX);
        data = new BulkNodeTxData(TxSchemas.BULK_EMPTY_ROOT_TX);
        expect(data.typeTag).toEqual(SignableTypeTags.BULK_EMPTY_ROOT_TX);
        data = new BulkNodeTxData(TxSchemas.BULK_ROOT_TX);
        expect(data.typeTag).toEqual(SignableTypeTags.BULK_ROOT_TX);
        data = new BulkNodeTxData(TxSchemas.BULK_NODE_TX);
        expect(data.typeTag).toEqual(SignableTypeTags.BULK_NODE_TX);
        data = new BulkNodeTxData(TxSchemas.BULK_TX);
        expect(data.typeTag).toEqual(SignableTypeTags.BULK_TX);
        data = new BulkNodeTxData(TxSchemas.UNITARY_TX);
        expect(data.typeTag).toEqual(SignableTypeTags.UNITARY_TX);
        expect(data.nonce.length).toEqual(8);
    });
    test('accessors', async () => {
        const data: BulkNodeTxData = new BulkNodeTxData();
    ...
    test('methods', async () => {
        // aSDASDASD
        let data: BulkNodeTxData;
        data = new BulkNodeTxData().setTypeTag('myType');
        expect(data.typeTag).toEqual('myType');
    ...
    test('to/from unnamedObject', async () => {
        ...
        const data = new BulkNodeTxData()
            ...
            .setDependsOn(testData.dependsOnHex)
            .setSignerPublicKey(keyPair.publicKey);
        expect(data.schema).toEqual(TxSchemas.BULK_NODE_TX);
        expect(data.typeTag).toEqual(SignableTypeTags.BULK_NODE_TX);
        const dataUnnamedObject = await data.toUnnamedObject();
        const data2 = new BulkNodeTxData();
        await data2.fromUnnamedObject(dataUnnamedObject);
    ...
    test('to/from Object', async () => {
        ...
        const data2 = new BulkNodeTxData();
        await data2.fromObject(dataObject);
        expect(data2).toEqual(data);
